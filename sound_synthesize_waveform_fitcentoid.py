...
iters = 100 # iterations for synthesis
opsound = 99;
oplayeri = 8;
...
subj = 1
cluster = [2,5]
target = np.array([1.9671  ,1.0795,    0.4756 ,   0.4118,   -0.1103,   -0.0350])
...
for ci in range(0,realbeta.shape[1]-1): 
  ...
  for i in range(iters):#range(len(out_fns)):
     ...
     with tf.GradientTape() as tape:
        ...
        mse = tf.reduce_mean(tf.square( tf.add(predtmp,-target)))

        #erms = tf.reshape(r_lamb*x_rms,[5,1])
        predrms =  tf.add(mse, r_lamb*x_rms)
