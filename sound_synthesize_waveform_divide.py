...
if Betastr == 'realbeta':
  ...
  betaname = mapping_dir+'subj'+str(subj)+'_'+roi+'_'+method+'_6comps_SV10'+dospeech+'_randvggish_'+oplayer+'.h5'
...
for ci in range(0,realbeta.shape[1]): 
  ...
  init_images2 = np.random.uniform(-1, 1, [16000,n_syn]).astype(np.float64)
  ...
  if Starter == 'randstarter':
    x = tf.convert_to_tensor(init_images,dtype=tf.float64)
    x2 = tf.convert_to_tensor(init_images2,dtype=tf.float64)
  ...
  for i in range(iters):#range(len(out_fns)):
     ...
     with tf.GradientTape() as tape:
        ...
        # if ci == 0:
        #  rate = tf.divide(pred[:,0],pred[:,1])
        # else:
        #  rate = tf.divide(pred[:,1],pred[:,0])
        if pattern =='onehot':
          predsm = tf.nn.softmax(pred)                      
          predsm = tf.cast(-predsm[:,ci],dtype=tf.float64)
          predtmp = tf.cast(predsm,dtype=tf.float64) 
        elif pattern == 'counter':
          #if ci == 0:
          pred1 = tf.reshape(pred[:,ci],[n_syn,]) 
          pred2 = tf.reshape(0.1*pred[:,~ci],[n_syn,])   
          # else:
          #  pred1 = tf.reshape(pred[:,1],[n_syn,]) 
          #  pred2 = tf.reshape(0.1*pred[:,0],[n_syn,]) 
          predtmp = tf.add(-pred1,pred2)  
          predtmp = tf.cast(predtmp,dtype=tf.float64) 
        
        #predrms =  tf.add(predtmp, r_lamb*x_rms)
        cost =  tf.add(predtmp, l_lamb*lv)
           
        print(f'cluster={cluster[ci]}, sound={1}, iters={i}, cost={cost.numpy()[0]}, pred={pred.numpy()[0,0]}')
        print(f'cluster={cluster[ci]}, sound={1}, iters={i}, cost={cost.numpy()[0]}, pred={pred.numpy()[0,1]}')

        grad = tape.gradient(cost, x)
        grad/= tf.math.reduce_std(grad,axis=[0,1],keepdims=True) + 1e-8
        x = x - lr*grad
        costeach[i]=cost.numpy() 
        predeach[i]=pred.numpy()[:,ci] 
        rmseach[i]=pred.numpy() [:,~ci] 
        lveach[i]=lv.numpy()
