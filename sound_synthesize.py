...
lr = 1e-1 # learing rate for synthesis   
iters = 700 # iterations for synthesis
opsound = 31;
oplayeri = 8;
...
subj = 3
cluster = 1
betaname = 'subj'+str(subj)+'_'+roi+'_'+method+'_cluster'+ str(cluster)+'_6comps.h5'
...
for j in range(1,5):
    ...
    for i in range(iters):#range(len(out_fns)):
        ...
        #x_jittered = transform(x_jittered.repeat([1, 3, 1, 1]))
            features = extract_features(model,x,oplayer)
        #features = tf.convert_to_tensor(features)
            ...
            #cost = tf.cast(pred,dtype=tf.float64)
            # predrms =  tf.add(pred, r_lamb*x_rms)
            # cost =  tf.add(pred, l_lamb*lv)
            
            print(f'iters = {i}, pred = {pred.numpy()}, cost = {cost.numpy()}')
            if cost < best_cost:
                best_x = x
                best_cost = cost
                best_pred = pred
 ...
 #   print(layer)
   ...
   #     os.remove(out_file)
    ...
    hf.create_dataset('new_mel',data=best_x)
    hf.close()
print('all done')
# In[]
