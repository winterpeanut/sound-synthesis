...
input_batch = vggish_input_tensor.waveform_to_examples(samples,sr)
...
for ci in range(0,realbeta.shape[1]): 
  ...
  for j in range(1,2):
    ...
    smeach = np.zeros([iters,1])
    rmseach = np.zeros([iters,1])
    lveach = np.zeros([iters,1])
    xeach = np.zeros([iters,16000])
    for i in range(iters):#range(len(out_fns)):
        ...
        with tf.GradientTape() as tape:
            ...
            predsm = tf.cast(-predsm[0,ci],dtype=tf.float64)
           
            predrms =  tf.add(predsm, r_lamb*x_rms)
            cost =  tf.add(predrms, l_lamb*lv)
            
            print(f'cluster={cluster[ci]}, sound={j}, iters={i}, cost={cost.numpy()}, pred={pred[0,ci].numpy()}, softmax={-predsm.numpy()}')
            if cost < best_cost:
                best_x = x
                best_cost = cost
                best_pred = pred
 ...
 #   print(layer)
    ...
    allx.append(xeach)
   # out_file=out_fns[14]
    out_dir_mel= 'D:\\EXP2\\Results\\DirectlyWaveform\\onehot\\'
  ...
  out_dir_mel= 'D:\\EXP2\\Results\\DirectlyWaveform\\Onehot\\logdata\\'
  ...
  hf.create_dataset('softmax',data=allsm)
